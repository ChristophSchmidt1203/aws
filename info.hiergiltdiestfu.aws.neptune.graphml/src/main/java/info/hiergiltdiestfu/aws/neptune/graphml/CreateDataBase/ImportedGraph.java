...
/**
 * Class which gets the Data as a XML of the backup from a rest or string. Then
 * unmarschalls the data into it's objects.
 * 
 * @author LUNOACK
 *
 */
public class ImportedGraph {
	...
	public String restToString() throws MalformedURLException {
		...
		URL url = new URL("http://localhost/graphML"); // Syntax aus Aplliprop
	...
	public void xmlfiletoEntity() {
		try {
			...
			setType((GraphmlType) ((JAXBElement<?>) unmarshaller.unmarshal(new FileInputStream(resource.getFile())))
					.getValue());
