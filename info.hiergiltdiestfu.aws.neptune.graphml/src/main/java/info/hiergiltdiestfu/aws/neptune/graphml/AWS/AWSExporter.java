...
public class AWSExporter {
	 ...
	 */
	public File createFileforUpload(String host, String port) {
	 ...
	 */
	public File writeFile(File tempfile, String host, String port) {
		logger.info("Write Database-Data on File...");

		try {
			new NeptuneAdapter(port, host).serialize(new FileWriter(tempfile));
		} catch (Exception e) {
