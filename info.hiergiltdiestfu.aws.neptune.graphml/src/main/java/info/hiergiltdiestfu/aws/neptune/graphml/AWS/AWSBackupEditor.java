...
/**
 * This class is there to see which backups must be kept and which can be
 * deleted. Backups are managed according to the time of their creation, keeping
 * the backups of the last 7 days and the Sunday backups of the last 6 weeks.
 * 
 * @author LUNOACK
 */
@Component
public class AWSBackupEditor {
	...
	public void deleteBackup() {
		logger.info("Starting Backup Clean-Up");
		...
		if (!keys.isEmpty()) {
			DeleteObjectsRequest deleteObjectsRequest = new DeleteObjectsRequest(awss3bucket).withKeys(keys)
					.withQuiet(false);
			logger.info("Delete...");
			amazons3.deleteObjects(deleteObjectsRequest);
			logger.info("Backups have been deleted.");
		} else {
			logger.info("All Backups are up-to-date.");
		}
	}

	/**
	 * Checks when the backups were created (Keep BackUps of the last 7 Days and
	 * keep each Sunday BackUp of the last 6 weeks)
	 * 
